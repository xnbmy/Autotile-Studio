...
- **导出模板**：图片导入的「是否预处理」界面提供「导出模板」按钮，一键将 516×516 布局模板导出到桌面，供 AI 生图参考
- 使用说明文档（含对齐微调说明）、悬停参数说明
...
> 近期主要更新的内容汇总（当前版本 v0.2.10，版本号见 `package.json`、`src-tauri/Cargo.toml` 与 `src-tauri/tauri.conf.json`）。

### v0.2.10 · 2026-08-19
- **导出模板**：图片导入的「是否预处理」界面新增「导出模板」按钮，一键将 516×516 布局模板导出到桌面，供 AI 生图参考。

### v0.2.9 · 2026-08-19
- **修复滑块等控件不可见**：Base UI 1.7.0 将方向数据属性由 `data-horizontal` / `data-vertical` 改为 `data-orientation`，导致参数面板的拖动条（腐蚀强度 / 边缘厚度等）只显示手柄、轨道不可见；已统一修正 slider / separator / scroll-area / tabs / toggle-group / field 六个组件的方向样式。

### v0.2.8 · 2026-08-19
- **修复中文文件名乱码**：导出「另存为」对话框输入中文文件名会乱码，根因是 PowerShell 子进程按系统代码页（GBK）输出 stdout、Rust 按 UTF-8 解码失败；已强制子进程 stdout 使用 UTF-8 编码。
