...
import { downloadCanvasAsPNG } from "@/lib/export-presets"
...
export function ExportDialog({ open, onOpenChange, asset }: { open: boolean; onOpenChange: (v: boolean) => void; asset: LibraryAsset | null }) {
  ...
  function handleExport() {
    ...
    downloadCanvasAsPNG(sheet.canvas, `${filename}.png`)

    toast.success("已导出标准排版 Tilesheet", { description: `${asset.tiles.size} 个图块 · ${sheet.width}×${sheet.height}px · 横向间距 ${spacingX}px · 纵向间距 ${spacingY}px` })
    onOpenChange(false)
  }
