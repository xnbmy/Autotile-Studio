...
export type ToolType = "brush" | "line" | "circle" | "fill" | "eraser" | "pan"
export type AppMode = "A" | "B"

// D2：三入口来源模式，与现有 AppMode（"A"|"B"）解耦。
// 映射：procedural→A（程序生成）、slice→B（导入切片）、draw→手绘（新增）。
export type SourceMode = "procedural" | "slice" | "draw"
